...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function deleteNote(_id) {
        ...
        if (!_id || typeof _id !== 'string' || _id.length !== 24) {
            console.error("ID proporcionado inválido: ", _id);
            return;
        }
    ...
    auth.onAuthStateChanged(user => {
        if (user) {
            user.getIdToken().then(idToken => {
                console.log('idtoken de firebase:', idToken);
                idTokenGlobal = idToken;
                loadNotes(idToken); // Se pasa el idToken como parámetro
            }).catch(error => {
                console.error('Error obteniendo el token de ID:', error);
            });
        } else {
            console.error('No hay ningún usuario actualmente autenticado.');
        }
    });
    ...
    function addNote(title, description) {
        if (!idTokenGlobal) {
            console.error('No se ha encontrado el idToken.');
            return;
        }
            ...
            fetch('https://localhost:7061/api/Note?idToken=' + idTokenGlobal, {
                ...
                body: JSON.stringify({ id: 'string', title: title, content: description ,userId: 'string', createAt:'createAt' })
            })
        ...
        .then(note => {
            ...
            loadNotes(idTokenGlobal); // Cargar las notas después de agregar una nueva nota
        })
        .catch(error => console.error('Error agregando la nota:', error));
    ...
    // Cargar las notas cuando la página se carga